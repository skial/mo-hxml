...
@:enum abstract RecognisedHxml(Int) from Int to Int {
	...
	public static #if !debug inline #end function all():Array<RecognisedHxml> {
		return [SourcePath, Main, Library, Define, DeadCode, Verbose, Debug, Cmd, Js, Swf, As3, Neko, Php, Cpp, Cs, Java, Python, Xml];
	}
	
	public #if !debug inline #end function asString():String {
		return switch (this) {
			case SourcePath: 'cp';
			case Main: 'main';
			case Library: 'lib';
	...
	public #if !debug inline #end function remove(mask:RecognisedHxml):RecognisedHxml {
		return new RecognisedHxml(this & ~mask);
	}
    
	public #if !debug inline #end function set(mask:RecognisedHxml):RecognisedHxml {
		return new RecognisedHxml(this | mask);
	}
    
	public #if !debug inline #end function exists(mask:RecognisedHxml):Bool {
		return this & mask != 0;
	}
    
    public #if !debug inline #end function new(v:Int) {
        this = v;
	}
